...
defmodule EventsSPA.Invites do
  ...
  def is_invited(post_id, user_id) do
    query = from x in "invites",
              where: x.post_id == ^post_id and x.user_id == ^user_id
    Repo.exists?(query)
  end

  def list_posts_by_invites(post_id) do
    query = from x in "invites",
              where: x.post == ^post_id
    Repo.all(query)
  end
